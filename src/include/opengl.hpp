// dear gui: Renderer Backend for modern OpenGL with shaders / programmatic
// pipeline
...
// About WebGL/ES:
// - You need to '#define IMPL_OPENGL_ES2' or '#define
// IMPL_OPENGL_ES3' to use WebGL or OpenGL ES.
// - This is done automatically on iOS, Android and Emscripten targets.
// - For other targets, the define needs to be visible from the
// gui_impl_opengl3.cpp compilation unit. If unsure, define globally or in
// config.hpp.
...
//  version table at the top of gui_impl_opengl3.cpp.

#pragma once
#include "gui.hpp" // IMPL_API
#ifndef DISABLE

// Backend API
IMPL_API bool ImplOpenGL3_Init(const char *glsl_version = nullptr);
IMPL_API void ImplOpenGL3_Shutdown();
IMPL_API void ImplOpenGL3_NewFrame();
IMPL_API void ImplOpenGL3_RenderDrawData(DrawData *draw_data);

// (Optional) Called by Init/NewFrame/Shutdown
IMPL_API bool ImplOpenGL3_CreateFontsTexture();
IMPL_API void ImplOpenGL3_DestroyFontsTexture();
IMPL_API bool ImplOpenGL3_CreateDeviceObjects();
IMPL_API void ImplOpenGL3_DestroyDeviceObjects();

// Specific OpenGL ES versions
// #define IMPL_OPENGL_ES2     // Auto-detected on Emscripten
// #define IMPL_OPENGL_ES3     // Auto-detected on iOS/Android

// You can explicitly select GLES2 or GLES3 API by using one of the '#define
// IMPL_OPENGL_LOADER_XXX' in config.hpp or compiler command-line.
#if !defined(IMPL_OPENGL_ES2) && !defined(IMPL_OPENGL_ES3)
...
#include <TargetConditionals.h>
    ...
    (defined(__ANDROID__))
#define IMPL_OPENGL_ES3 // iOS, Android  -> GL ES 3, "#version 300 es"
#elif defined(__EMSCRIPTEN__) || defined(__amigaos4__)
#define IMPL_OPENGL_ES2 // Emscripten    -> GL ES 2, "#version 100"
#else
// Otherwise gui_impl_opengl3_loader.hpp will be used.
#endif
...
//  2023-11-08: OpenGL: Update GL3W based gui_impl_opengl3_loader.hpp to load
//  "libGL.so" instead of "libGL.so.1", accomodating for NetBSD systems having
//  only "libGL.so.3" available. (#6983) 2023-10-05: OpenGL: Rename symbols in
//  our internal loader so that LTO compilation with another copy of gl3w is
...
//  IMPL_OPENGL_DEBUG'. 2022-05-23: OpenGL: Reworking 2021-12-15 "Using
...
//  minimal GL loader (gui_impl_opengl3_loader.hpp), removing requirement and
...
//  version. 2021-05-19: OpenGL: Replaced direct access to DrawCmd::TextureId
//  with a call to DrawCmd::GetTexID(). (will become a requirement)
...
//  BackendFlags_RendererHasVtxOffset. 2020-03-24: OpenGL: Added support
...
//  calling ImplOpenGL3_CreateFontsTexture() before the first NewFrame()
...
//  2019-03-15: OpenGL: Added a GL call + comments in ImplOpenGL3_Init()
...
//  gui_impl_opengl3.cpp/.h away from the old combined GLFW/SDL+OpenGL3
...
//  Added const char* glsl_version parameter to ImplOpenGL3_Init() so user
...
//  io.RenderDrawListsFn callback and exposed ImplSdlGL3_RenderDrawData()
...
#endif
#elif defined(IMPL_OPENGL_ES3)
#if (defined(__APPLE__) && (TARGET_OS_IOS || TARGET_OS_TV))
#include <OpenGLES/ES3/gl.h> // Use GL ES 3
#else
#include <GLES3/gl3.h> // Use GL ES 3
#endif
#elif !defined(IMPL_OPENGL_LOADER_CUSTOM)
...
// feature for this backend (gui_impl_opengl3.cpp):
// - You may need to regenerate gui_impl_opengl3_loader.hpp to add new symbols.
...
#define IMGL3W_IMPL
//-----------------------------------------------------------------------------
// About gui_impl_opengl3_loader.hpp:
//
...
// enums/functions that we use in our gui_impl_opengl3.cpp source file in
// order to be small.
//
// YOU SHOULD NOT NEED TO INCLUDE/USE THIS DIRECTLY. THIS IS USED BY
// gui_impl_opengl3.cpp ONLY. THE REST OF YOUR APP SHOULD USE A DIFFERENT GL
...
// 'gui_impl_opengl3.cpp' OR INCUDING 'gui_impl_opengl3_loader.hpp' IN THE
...
// IMPL_OPENGL_LOADER_CUSTOM' and gui_impl_opengl3.cpp WILL NOT BE USING
// OUR LOADER, AND INSTEAD EXPECT ANOTHER/YOUR LOADER TO BE AVAILABLE IN THE
// COMPILATION UNIT.
//
// Regenerate with:
//   python gl3w_gen.py --output ../imgui/backends/gui_impl_opengl3_loader.hpp
//   --ref ../imgui/backends/gui_impl_opengl3.cpp ./extra_symbols.txt
//
// More info:
...
inline int imgl3wInit2(GL3WGetProcAddressProc proc) {
  load_procs(proc);
  return parse_version();
}

inline int imgl3wIsSupported(int major, int minor) {
  if (major < 2)
    return 0;
...
inline GL3WglProc imgl3wGetProcAddress(const char *proc) {
...
// extension
#ifndef IMPL_OPENGL_ES2
#define IMPL_OPENGL_USE_VERTEX_ARRAY
...
#define IMPL_OPENGL_MAY_HAVE_VTX_OFFSET
#endif

// Desktop GL 3.3+ and GL ES 3.0+ have glBindSampler()
#if !defined(IMPL_OPENGL_ES2) &&                                               \
    (defined(IMPL_OPENGL_ES3) || defined(GL_VERSION_3_3))
#define IMPL_OPENGL_MAY_HAVE_BIND_SAMPLER
#endif

// Desktop GL 3.1+ has GL_PRIMITIVE_RESTART state
#if !defined(IMPL_OPENGL_ES2) && !defined(IMPL_OPENGL_ES3) &&                  \
    defined(GL_VERSION_3_1)
#define IMPL_OPENGL_MAY_HAVE_PRIMITIVE_RESTART
#endif

// Desktop GL use extension detection
#if !defined(IMPL_OPENGL_ES2) && !defined(IMPL_OPENGL_ES3)
#define IMPL_OPENGL_MAY_HAVE_EXTENSIONS
#endif

// [Debugging]
// #define IMPL_OPENGL_DEBUG
#ifdef IMPL_OPENGL_DEBUG
#include <stdio.h>
...
struct ImplOpenGL3_Data {
  ...
  ImplOpenGL3_Data() { memset((void *)this, 0, sizeof(*this)); }
};
...
static ImplOpenGL3_Data *ImplOpenGL3_GetBackendData() {
  return Gui::GetCurrentContext()
             ? (ImplOpenGL3_Data *)Gui::GetIO().BackendRendererUserData
             : nullptr;
}

// OpenGL vertex attribute state (for ES 1.0 and ES 2.0 only)
#ifndef IMPL_OPENGL_USE_VERTEX_ARRAY
struct ImplOpenGL3_VtxAttribState {
...
// Functions
inline bool ImplOpenGL3_Init(const char *glsl_version) {
  IO &io = Gui::GetIO();
...
#if !defined(IMPL_OPENGL_ES2) && !defined(IMPL_OPENGL_ES3) &&                  \
    !defined(IMPL_OPENGL_LOADER_CUSTOM)
  if (imgl3wInit() != 0) {
  ...
  ImplOpenGL3_Data *bd = NEW(ImplOpenGL3_Data)();
  io.BackendRendererUserData = (void *)bd;
  io.BackendRendererName = "gui_impl_opengl3";
...
#ifdef IMPL_OPENGL_MAY_HAVE_VTX_OFFSET
  if (bd->GlVersion >= 320)
    io.BackendFlags |=
...
#ifdef IMPL_OPENGL_MAY_HAVE_EXTENSIONS
...
inline void ImplOpenGL3_Shutdown() {
  ImplOpenGL3_Data *bd = ImplOpenGL3_GetBackendData();
  ASSERT(bd != nullptr &&
...
inline void ImplOpenGL3_NewFrame() {
  ImplOpenGL3_Data *bd = ImplOpenGL3_GetBackendData();
  ASSERT(bd != nullptr && "Did you call ImplOpenGL3_Init()?");

  if (!bd->ShaderHandle)
    ImplOpenGL3_CreateDeviceObjects();
}

static void ImplOpenGL3_SetupRenderState(DrawData *draw_data, int fb_width,
                                         int fb_height,
...
#ifdef IMPL_OPENGL_MAY_HAVE_PRIMITIVE_RESTART
  if (bd->GlVersion >= 310)
    glDisable(GL_PRIMITIVE_RESTART);
#endif
#ifdef IMPL_HAS_POLYGON_MODE
  ...
  // Our visible gui space lies from draw_data->DisplayPos (top left) to
...
#ifdef IMPL_OPENGL_MAY_HAVE_BIND_SAMPLER
...
#ifdef IMPL_OPENGL_USE_VERTEX_ARRAY
  glBindVertexArray(vertex_array_object);
#endif

  // Bind vertex/index buffers and setup attributes for DrawVert
...
inline void ImplOpenGL3_RenderDrawData(DrawData *draw_data) {
  ...
  ImplOpenGL3_VtxAttribState last_vtx_attrib_state_pos;
  last_vtx_attrib_state_pos.GetState(bd->AttribLocationVtxPos);
  ImplOpenGL3_VtxAttribState last_vtx_attrib_state_uv;
  last_vtx_attrib_state_uv.GetState(bd->AttribLocationVtxUV);
  ImplOpenGL3_VtxAttribState last_vtx_attrib_state_color;
...
#endif
  ImplOpenGL3_SetupRenderState(draw_data, fb_width, fb_height,
                               vertex_array_object);
  ...
  for (int n = 0; n < draw_data->CmdListsCount; n++) {
    ...
    const GLsizeiptr vtx_buffer_size =
        (GLsizeiptr)cmd_list->VtxBuffer.Size * (int)sizeof(DrawVert);
    const GLsizeiptr idx_buffer_size =
        (GLsizeiptr)cmd_list->IdxBuffer.Size * (int)sizeof(DrawIdx);
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        if (bd->GlVersion >= 320)
          GL_CALL(glDrawElementsBaseVertex(
              ...
              (void *)(intptr_t)(pcmd->IdxOffset * sizeof(DrawIdx)),
...
inline bool ImplOpenGL3_CreateFontsTexture() {
  IO &io = Gui::GetIO();
  ImplOpenGL3_Data *bd = ImplOpenGL3_GetBackendData();
...
inline void ImplOpenGL3_DestroyFontsTexture() {
  IO &io = Gui::GetIO();
  ImplOpenGL3_Data *bd = ImplOpenGL3_GetBackendData();
  if (bd->FontTexture) {
...
static bool CheckShader(GLuint handle, const char *desc) {
  ...
  if ((GLboolean)status == GL_FALSE)
    fprintf(stderr,
            "ERROR: ImplOpenGL3_CreateDeviceObjects: failed to compile "
            "%s! With GLSL: %s\n",
...
static bool CheckProgram(GLuint handle, const char *desc) {
  ...
  if ((GLboolean)status == GL_FALSE)
    fprintf(stderr,
            "ERROR: ImplOpenGL3_CreateDeviceObjects: failed to link %s! "
            "With GLSL %s\n",
            desc, bd->GlslVersionString);
  if (log_length > 1) {
    Vector<char> buf;
...
inline bool ImplOpenGL3_CreateDeviceObjects() {
  ImplOpenGL3_Data *bd = ImplOpenGL3_GetBackendData();
...
inline void ImplOpenGL3_DestroyDeviceObjects() {
  ImplOpenGL3_Data *bd = ImplOpenGL3_GetBackendData();
  if (bd->VboHandle) {
